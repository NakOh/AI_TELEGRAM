...
const ChatList = ({
  ...
  const rawOrderedIds = useFolderManagerForOrderedIds(resolvedFolderId);
  usePeerStoriesPolling(rawOrderedIds);

  const { isHideForwardedMessages } = useHideForwarded();

  const debouncedFilteredIds = useDebouncedMemo(() => {
  ...
  const orderedIds = isHideForwardedMessages ? debouncedFilteredIds : rawOrderedIds;
